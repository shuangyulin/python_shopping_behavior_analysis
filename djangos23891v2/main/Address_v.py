...
def address_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        # 当前表也是有管理员权限的表
        if  __isAdmin__ == "是" and 'address' != 'forum' :
            if req_dict.get("userid") and 'address' != 'chat' and 'address' != 'examrecord':
                del req_dict["userid"]
        else:
        ...
        msg['data']['pageSize']  =address.page(address, address, req_dict, request, q)
        return JsonResponse(msg, encoder=CustomJsonEncoder)
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        ...
        elif "browseduration"  in address.getallcolumn(address,address):
...
#分类列表
def address_lists(request):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code,  "data":[]}
        msg['data'],_,_,_,_  = address.page(address, address, {})
        return JsonResponse(msg, encoder=CustomJsonEncoder)
...
def address_query(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        try:
            query_result = address.objects.filter(**request.session.get("req_dict")).values()
...
def address_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        idOrErr= address.createbyreq(address,address, req_dict)
...
def address_update(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if 'clicktime' in req_dict.keys() and req_dict['clicktime']=="None":
            del req_dict['clicktime']
        if req_dict.get("mima") and "mima" not in address.getallcolumn(address,address) :
            del req_dict["mima"]
        if req_dict.get("password") and "password" not in address.getallcolumn(address,address) :
...
def address_security(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT * FROM address where ='{0}'".format(req_dict['username'])
        record = {}
