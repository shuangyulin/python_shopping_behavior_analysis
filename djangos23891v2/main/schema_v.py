...
def schemaName_spider(request, tableName):
    if request.method in ["POST", "GET"]:
        ...
        # Linux
        cmd = "cd /yykj/python/9999/spider76q4qml1 && scrapy crawl "+tableName+"Spider -a databaseName=djangos23891v2"
        # Windows
        # cmd = "cd C:\\test1\\spider && scrapy crawl " + tableName + "Spider -a databaseName=djangos23891v2"
        os.system(cmd)
...
def schemaName_mysqldump(request):
    if request.method in ["POST", "GET"]:
        ...
        filePath = os.path.join(os.getcwd(), "templates/front", "backup.sql")
        cmd = "/usr/bin/mysqldump -h127.0.0.1 -uroot -P3306 -p123456 djangos23891v2 > " + filePath
        os.system(cmd)
