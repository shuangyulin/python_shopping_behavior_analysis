...
def yonghu_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        if timeStatType == '年':
            date_type = "year"
        json_filename = f'yonghu_value{xColumnName}{yColumnName}{date_type}.json'
...
def yonghu_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        #获取hadoop分析后的数据文件
        json_filename = f'yonghu_value{xColumnName}{yColumnName}.json'
...
def yonghu_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        ...
        if timeStatType == '年':
            date_type = "year"
        #获取hadoop分析后的数据文件
        json_filename = f'yonghu_value{xColumnName}｛yColumnNameMul.replace(",","")｝{date_type}.json'
...
def yonghu_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        ...
        req_dict = request.session.get("req_dict")
        #获取hadoop分析后的数据文件
        json_filename = f'yonghu_value{xColumnName}｛yColumnNameMul.replace(",","")｝.json'
...
def yonghu_group(request, columnName):
    if request.method in ["POST", "GET"]:
        ...
        #获取hadoop分析后的数据文件
        json_filename = f'yonghu_group{columnName}.json'
